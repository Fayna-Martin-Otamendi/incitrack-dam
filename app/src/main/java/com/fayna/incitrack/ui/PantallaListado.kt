...
// Pantalla donde se muestra el listado general de incidencias.
// Desde aquí se pueden aplicar filtros por texto, fecha, estado, categoría y tipo.
...
class PantallaListado : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        etBuscar = findViewById(R.id.etBuscarListado)
        etFecha = findViewById(R.id.etFechaListado)
        spEstado = findViewById(R.id.spEstadoListado)
        spCategoria = findViewById(R.id.spCategoriaListado)
        spTipo = findViewById(R.id.spTipoListado)
        btnFiltrar = findViewById(R.id.btnFiltrarListado)
        btnLimpiar = findViewById(R.id.btnLimpiarListado)
    ...
    // y en modo administrador las ordena por prioridad
    private fun cargarLista() {
        val dbHelper = DatabaseHelper(this)
        val incidenciaDAO = IncidenciaDAO(dbHelper)
        val categoriaDAO = CategoriaDAO(dbHelper)

        listaIncidenciasOriginal = ArrayList(incidenciaDAO.getAllIncidencias())
        ...
        if (modoAdmin) {
            for (incidencia in listaIncidenciasOriginal) {
                ...
                incidencia.setPrioridadCalculada(prioridadReal)
            }

            listaIncidenciasOriginal.sortByDescending { it.getPrioridadCalculada() }
        }
    ...
    private fun aplicarFiltros() {
        ...
        // En modo administrador también se mantiene la ordenación por prioridad
        if (modoAdmin) {
            listaFiltrada.sortByDescending { it.getPrioridadCalculada() }
        }
    ...
    private fun mostrarLista(lista: List<Incidencia>) {
        val adapter = IncidenciaAdapter(this, lista, "LISTADO", modoAdmin, idUsuario)
